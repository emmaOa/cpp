...
/*   Updated: 2023/01/23 18:56:10 by iouazzan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::attack(const std::string& target)
{
    if (hit_points <= 0 || eng_points <= 0)
        return;
    eng_points--;
    std::cout << "ClapTrapk " << name << " attacks " << \
    target << ", causing " << att_damage << " points of damage!\n";
}
...
{
    if (this->hit_points <= 0 || this->eng_points <= 0)
        return;
